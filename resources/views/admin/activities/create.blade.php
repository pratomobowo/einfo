...
<div class="space-y-6">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-semibold text-gray-900">{{ __('Tambah Kegiatan Baru') }}</h1>
            <p class="mt-2 text-sm text-gray-700">{{ __('Isi formulir di bawah untuk menambahkan kegiatan baru') }}</p>
        </div>
    </div>
    ...
    <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white">
            <form action="{{ route('admin.activities.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                <x-activities.form :officials="$officials" />
